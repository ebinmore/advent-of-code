...
# want to:
#  - get minimum number of cubes per game (ie. the max for each colour)
#  - multiply min cubes per colour per game
#  - sum all game totals
games.values.each do |game|
    game[:total] = (game[:maximums][:red] || 1) *
        (game[:maximums][:green] || 1) *
        (game[:maximums][:blue] || 1)
end

byebug

sum_of_game_totals = games.values.reduce(0) do |memo, game| 
    puts "game[:total] = #{game[:total]}" 
    memo += game[:total]
    puts "memo = #{memo}"
    memo
end

puts "Total from Games:"
puts sum_of_game_totals

# Total from Games:
# 2286
# WRONG -- too low -- that's the test data
# Total from Games:
# 84538